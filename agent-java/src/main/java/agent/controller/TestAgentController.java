...
/**
 * REST API controller for external invocation of the UI test agent. Provides endpoints for n8n workflow integration and
...
public class TestAgentController {
  ...
  /**
   * Generate and run a UI test using configurable generation mode.
   *
   * <p>
   * Request body:
   *
   ...
   *   "mode": "STEP_BY_STEP",          // Optional: STEP_BY_STEP (default), FAST, or AUTO
   *   "runAccessibilityAudit": false,   // Optional: Run WCAG accessibility check
   *   "createPullRequest": false        // Optional: Create PR with generated test
   * }
   * }</pre>
   *
   * <p>
   * Generation modes:
   * <ul>
   * <li>{@code STEP_BY_STEP} - Analyzes DOM+screenshot per step. Most
   * accurate.</li>
   * <li>{@code FAST} - Monolithic generation. Faster for simple tests.</li>
   * <li>{@code AUTO} - Selects based on complexity (≤2 steps = FAST, else
   * STEP_BY_STEP)</li>
   * </ul>
   */
  @PostMapping("/generate")
  public ResponseEntity<TestExecutionResult> generateTest(@RequestBody TestGenerationRequest request) {
    log.info("Received test generation request: {} with {} steps, mode: {}",
        request.title(), request.steps().size(), request.effectiveMode());
  ...
  /**
   * Analyze test steps without executing - returns planned Playwright instructions. Useful for debugging and previewing
   * what actions the LLM would generate.
   */
  @PostMapping("/analyze")
  ...
  /**
   * n8n-compatible webhook endpoint. Accepts flexible payload format and returns structured response.
   * <p>
   * Expected payload: { "testCase": { "title": "Test title", "steps": ["step1", "step2"] }, "url":
   * "https://target-site.com" }
   * <p>
   * OR simplified: { "title": "Test title", "steps": ["step1", "step2"], "url": "https://target-site.com" }
   */
